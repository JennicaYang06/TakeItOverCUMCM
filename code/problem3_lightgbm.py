...
from problem3 import read_attachment3, LOCK_LEN, EPOCHS
...
def main():
    ...
    ap.add_argument("--min-train-rows", type=int, default=100, help="负荷LightGBM最少训练样本数，不足则用前一天实际值兜底")
    ap.add_argument("--outdir", default=None, help="输出目录，默认 results/problem3_lightgbm")
    args = ap.parse_args()
    ...
    print(f"负荷LightGBM冷启动回退天数: {n_fallback}（历史有效训练样本 < {args.min_train_rows} 行时用前一天实际值兜底）")
    ...
    lines.append(f"日内调整: {'开启（6/12/18点联合剩余日重优化，只锁定接下来6小时）' if use_adjustment else '关闭（只执行0点计划）'}")
